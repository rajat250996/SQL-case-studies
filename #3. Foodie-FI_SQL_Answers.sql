...
(
SELECT S.CUSTOMER_ID, S.PLAN_ID, P.PLAN_NAME, S.START_DATE,
ROW_NUMBER() OVER(PARTITION BY S.CUSTOMER_ID ORDER BY S.START_DATE),
CASE
	WHEN P.PLAN_NAME LIKE 'pro annual' THEN 1
	ELSE 0
END AS annual_plan_flag
FROM SUBSCRIPTIONS AS S
JOIN PLANS AS P
ON S.PLAN_ID = P.PLAN_ID
WHERE DATE_PART('YEAR', S.START_DATE) = 2020
...
SELECT SUM(annual_plan_flag) AS annual_upgraders
FROM CTE

-- *A*N*O*T*H*E*R E*A*S*Y A*P*P*R*O*A*C*H* --
SELECT COUNT(DISTINCT CUSTOMER_ID) AS annual_upgraders
FROM SUBSCRIPTIONS
...
SELECT AVG(DATE_PART('DAY', NEXT_PLAN_DATE::TIMESTAMP - START_DATE::TIMESTAMP)) AS avg_days
FROM CTE
WHERE NEXT_PLAN_DATE IS NOT NULL
...
BINS AS
(
SELECT WIDTH_BUCKET(NEXT_PLAN_DATE::TIMESTAMP - START_DATE::TIMESTAMP, 0, 365, 12)
